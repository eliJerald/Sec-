...
### Session Date: 2026-08-20 20:01:45 -07:00

#### Session Summary Report

**1. CompTIA Security+ SY0-701 Tutoring: PKI Deep Dive (Domain 3)**
* **OIDs and DNs:** Explored Object Identifiers (OIDs) and how they function as universal numbered labels for elements within an X.509 Certificate. Clarified how OIDs map directly to the various attributes of a Distinguished Name (DN).
* **Certificate Validation Levels:** Discussed Domain Validation (DV), Organization Validation (OV), and Extended Validation (EV), and explained why some certificates have mostly empty DNs while others are fully detailed based on the background check rigor.
* **Certificate Signing Requests (CSR):** Identified the CSR as the cryptographic file generated by an entity to request a certificate from a Certificate Authority (CA).
* **SAN vs. Wildcard Certificates:** Differentiated between Subject Alternative Name (SAN) certificates (for securing completely different domain names) and Wildcard certificates (for securing unlimited subdomains under a single main domain).

**2. CompTIA Security+ SY0-701 Tutoring: Threat Vectors (Domain 2)**
* **Removable Devices:** Identified malware delivery and data exfiltration as the primary direct physical threats of removable media like USB drives.
* **Client-based vs. Agentless Threat Vectors:** Clarified that client-based vectors (like malicious macros and drive-by downloads) exploit software directly on the endpoint, while agentless vectors (like network protocol vulnerabilities and packet sniffing) exploit network pathways without requiring software installation on the victim machine.

**3. Practice Questions Logged (`sec+_practice_Qs.md`)**
* **File Updates:** Continuously appended detailed Q&A blocks and a summary of the PKI deep dive to the study guide.

#### Next Steps & Future Work
* **Continue Question Bank:** Keep adding practice questions to the `sec+_practice_Qs.md` study guide.
* **Review PKI Concepts:** Review the newly added PKI summary in the study guide to reinforce the relationships between CSR, DN, OID, SAN, and Wildcard certificates.

--------------------------------------------------------------------------------

### Session Date: 2026-08-20 20:17:29 -07:00

#### Session Summary Report

**1. CompTIA Security+ SY0-701 Study Guides**
* **Comprehensive Guide:** Parsed the exam objectives PDF and the practice questions markdown file to create a comprehensive study guide covering all domains (1-5).
* **Domain 1 Deep Dive:** Created a detailed, focused study guide specifically for Domain 1 (General Security Concepts), expanding on security controls, cryptography, PKI, physical security, and Zero Trust.

**2. File Operations**
* **Artifact Relocation:** Moved the comprehensive study guide from the internal artifact directory directly to the workspace (`/mnt/d/AI projects/sec+/Security_Plus_SY0_701_Study_Guide.md`) per user request.
* **New File Creation:** Saved the Domain 1 deep-dive guide directly to the workspace (`/mnt/d/AI projects/sec+/Domain_1_Study_Guide.md`).

#### Next Steps & Future Work
* **Review Domain 1:** Thoroughly review the Domain 1 study guide to master foundational concepts like security controls and cryptography.
* **Domain 2-5 Guides:** Create specific deep-dive study guides for Domains 2 through 5 as requested by the user.
